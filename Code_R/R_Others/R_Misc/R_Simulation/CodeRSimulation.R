...
plot(x1, x2, col = "blue", main = "Strong Collinearity") # notice we have strong, but not perfect, multicollinearity
...
eps <- rnorm(1000, mean = 0, sd = sqrt(hetero.func(x))) # we produce epsilon from function of x
eps2 <- eps^2
y <- beta1 + beta2 * x + eps
reg.hetero <- lm(y ~ x)
